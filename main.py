...
'''
ZP's notes:
    For this main.py, I modified the  default model loading path
    so that the newly trained model could be load in testing.
...
def parse_args(args=None):
    ...
    parser.add_argument('--load_model_path', default='logs/checkpoint.pth', type=str, help='trained model checkpoint path.')

    # Train Params
    parser.add_argument('--batch_size', default=512, type=int, help='training batch size.')
    parser.add_argument('--max_epochs', default=101, type=int, help='max training epochs.')
    parser.add_argument('--num_workers', default=8, type=int, help='workers number used for dataloader.')
    parser.add_argument('--valid_epoch', default=34, type=int, help='validation frequency.')
    ...
    parser.add_argument('--weight', default=0.05, type=float, help='base weight for rule based reward')
...
def get_model_config(args, num_ent, num_rel):
    config = {
        ...
        'rules_path':'data/ICEWS14/rule1.json',
        'weight':args.weight
    }
    return config
...
def main(args):
    ...
    # creat the agent and load the config to Agent
    agent = Agent(config)

    # creat the environment
    state_actions_path = os.path.join(args.data_path, args.state_actions_path) # state action space
    ...
    if args.do_train: # training entry
