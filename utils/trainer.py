    ...
    def train_epoch(self, dataloader, ntriple):
        ...
        with tqdm.tqdm(total=ntriple, unit='ex') as bar:
            ...
            for src_batch, rel_batch, dst_batch, time_batch in dataloader:
                ...
                all_loss, all_logits, all_relations, current_entities, current_time = self.model(src_batch, time_batch, rel_batch)

                rule_reward = self.pg.get_rule_reward(all_relations).cuda()
                s_reward = self.pg.get_reward(current_entities, dst_batch)
                #print(rule_reward,type(rule_reward))
                reward = (1 + rule_reward)*s_reward
                # 对比答案计算 初级 01 reward
